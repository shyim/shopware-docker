...
cat /etc/nginx/proxy.tpl | envsubst > /etc/nginx/sites-enabled/mail.conf
...
then
    ...
    cat /etc/nginx/proxy.tpl | envsubst > /etc/nginx/sites-enabled/db.conf
fi
...
then
    ...
    cat /etc/nginx/proxy.tpl | envsubst > /etc/nginx/sites-enabled/cerebro.conf

    export PROXY_DOMAIN="es"
    export PROXY_HOST="elastic"
    export PROXY_PORT="9200"
    cat /etc/nginx/proxy.tpl | envsubst > /etc/nginx/sites-enabled/es.conf
fi
